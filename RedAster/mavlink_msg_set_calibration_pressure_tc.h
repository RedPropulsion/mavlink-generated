 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_set_calibration_pressure_tc_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               float pressure)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_SET_CALIBRATION_PRESSURE_TC_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_SET_CALIBRATION_PRESSURE_TC_MIN_LEN, MAVLINK_MSG_ID_SET_CALIBRATION_PRESSURE_TC_LEN, MAVLINK_MSG_ID_SET_CALIBRATION_PRESSURE_TC_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_SET_CALIBRATION_PRESSURE_TC_MIN_LEN, MAVLINK_MSG_ID_SET_CALIBRATION_PRESSURE_TC_LEN);
#endif
}
...
/**
 * @brief Encode a set_calibration_pressure_tc struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param set_calibration_pressure_tc C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_set_calibration_pressure_tc_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_set_calibration_pressure_tc_t* set_calibration_pressure_tc)
{
    return mavlink_msg_set_calibration_pressure_tc_pack_status(system_id, component_id, _status, msg,  set_calibration_pressure_tc->pressure);
}
