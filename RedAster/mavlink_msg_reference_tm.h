 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_reference_tm_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               uint64_t timestamp, float ref_altitude, float ref_pressure, float ref_temperature, float ref_latitude, float ref_longitude, float msl_pressure, float msl_temperature)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_REFERENCE_TM_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_REFERENCE_TM_MIN_LEN, MAVLINK_MSG_ID_REFERENCE_TM_LEN, MAVLINK_MSG_ID_REFERENCE_TM_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_REFERENCE_TM_MIN_LEN, MAVLINK_MSG_ID_REFERENCE_TM_LEN);
#endif
}
...
/**
 * @brief Encode a reference_tm struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param reference_tm C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_reference_tm_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_reference_tm_t* reference_tm)
{
    return mavlink_msg_reference_tm_pack_status(system_id, component_id, _status, msg,  reference_tm->timestamp, reference_tm->ref_altitude, reference_tm->ref_pressure, reference_tm->ref_temperature, reference_tm->ref_latitude, reference_tm->ref_longitude, reference_tm->msl_pressure, reference_tm->msl_temperature);
}
