 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_gs_discovery_request_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               uint32_t source_ip, uint16_t source_port)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_GS_DISCOVERY_REQUEST_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_GS_DISCOVERY_REQUEST_MIN_LEN, MAVLINK_MSG_ID_GS_DISCOVERY_REQUEST_LEN, MAVLINK_MSG_ID_GS_DISCOVERY_REQUEST_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_GS_DISCOVERY_REQUEST_MIN_LEN, MAVLINK_MSG_ID_GS_DISCOVERY_REQUEST_LEN);
#endif
}
...
/**
 * @brief Encode a gs_discovery_request struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param gs_discovery_request C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_gs_discovery_request_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_gs_discovery_request_t* gs_discovery_request)
{
    return mavlink_msg_gs_discovery_request_pack_status(system_id, component_id, _status, msg,  gs_discovery_request->source_ip, gs_discovery_request->source_port);
}
